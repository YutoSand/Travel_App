...
class TravelApp:
    ...
    def display_answer_options(self):
        ...
        if self.current_question_index > 0:
            back_button = tk.Button(self.answers_frame, text="戻る", command=self.go_back, font=("familly", 20), borderwidth=5, width=10, height=2)
            back_button.pack(side=tk.LEFT)

    def go_back(self):
        # 前の質問に戻る
        self.answers.pop()  # 直前の選択を削除
    ...
    def select_answer(self, answer_index):
        ...
        if self.current_question_index < len(questions):
            ...
            if is_random:
                selected_prefecture = next(prefecture for prefecture in prefectures if prefecture.name == recommendation)
                webbrowser.open(selected_prefecture.url, new=2)
                messagebox.showinfo("診断結果", f"条件に合致する都道府県が見つかりませんでした。\nしかし、厚かましいですが適当に都道府県を決定しました！\n参考にしてください。\nおすすめの旅行先は、　{recommendation} 　です。")
                
            else:
                # Access the URL from the selected prefecture
                selected_prefecture = next(prefecture for prefecture in prefectures if prefecture.name == recommendation)
                webbrowser.open(selected_prefecture.url, new=2)
